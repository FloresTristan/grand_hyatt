...
"[project]/.next-internal/server/app/api/admin/events/[id]/route/actions.js [app-rsc] (server actions loader, ecmascript)": ((__turbopack_context__) => {
...
"[project]/src/app/api/admin/events/[id]/route.ts [app-route] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
__turbopack_context__.s({
    ...
    "GET": ()=>GET,
    "PATCH": ()=>PATCH,
    "ensureUtcIso": ()=>ensureUtcIso,
    "runtime": ()=>runtime,
    "toStoragePath": ()=>toStoragePath
});
...
async function GET(_req, { params }) {
    ...
    const { data, error } = await supabase.from('events_with_status').select('*').eq('id', id).single();
...
async function DELETE(_req, { params }) {
    const { id } = await params;
    const supabase = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$supabase$2f$server$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["createSupabaseServer"])();
    const { data: ev, error: readErr } = await supabase.from('events').select('image_url').eq('id', id).single();
    if (readErr) return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
...
function toStoragePath(urlOrPath, bucket = 'events') {
    ...
    try {
        ...
        const m = u.pathname.match(new RegExp(`/object/(?:public|sign)/${bucket}/(.+)$`));
        if (m && m[1]) return m[1];
    } catch  {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__91b3b018._.js.map